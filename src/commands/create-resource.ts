...
export function createResource(inputName: string) {
  ...
  createDir(BASE_PATH);
  createFile(
    path.join(BASE_PATH, "page.tsx"),
    listPageTemplate(capitalize(resource))
  );
  ...
  createFile(
    ...
    detailPageTemplate(capitalize(singular))
  );
  ...
  createFile(
    ...
    newPageTemplate(capitalize(singular))
  );

  console.log(`Recurso "${resource}" criado com sucesso` + "\x1b[32m✔ Sucesso\x1b[0m");
}
