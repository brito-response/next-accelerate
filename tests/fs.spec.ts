...
describe("utils/fs", () => {
  ...
  it("moveFile should move file when origin exists and destination dir exists", () => {
    // given
    mockedFs.existsSync.mockReturnValueOnce(true).mockReturnValueOnce(true);
    moveFile("a.txt", "dest/a.txt"); // when 
    expect(mockedFs.renameSync).toHaveBeenCalledWith("a.txt", "dest/a.txt"); //then
  });

  it("moveFile should create destination dir if it doesn't exist", () => {
    // given
    ...
    expect(mockedFs.renameSync).toHaveBeenCalledWith("a.txt", "dest/a.txt");
  });

  it("moveFile should do nothing if origin file does not exist", () => {
    mockedFs.existsSync.mockReturnValue(false);  // given
    moveFile("a.txt", "dest/a.txt"); // when 
    // then
